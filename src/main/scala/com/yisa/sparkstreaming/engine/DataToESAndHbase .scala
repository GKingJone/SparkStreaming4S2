...
object DataToESAndHbase {
  ...
  def main(args: Array[String]) {
    ...
    // ES的数据库
    var esDataBaseName = "vehicle201704/info"

    var esDataBaseNames = "201704,201703,201702,201701,201612,201611"
    ...
    messages.foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        ...
        if (aaCount > 0) {
          ...
          var jsonEs = aaEs.map { x =>
            val gsonEs = new Gson
            val mapTypeEs = new TypeToken[PassInfoForEs] {}.getType
            gsonEs.toJson(x)
          }
          EsSpark.saveJsonToEs(jsonEs, esDataBaseName,Map("es.mapping.id" -> "A001"))

          //          aaEs.unpersist(true)
          //          jsonEs.unpersist(true)

          //          scES.stop()
          // 入ES-End

          // 入HBase-Start-新API
          sc.hadoopConfiguration.set("hbase.zookeeper.quorum", "gpu3,gpu5,gpu9")
...
//
//          val jobFirstCity = new Job(sc.hadoopConfiguration)
//          jobFirstCity.setOutputKeyClass(classOf[ImmutableBytesWritable])
//          jobFirstCity.setOutputValueClass(classOf[Result])
//          jobFirstCity.setOutputFormatClass(classOf[TableOutputFormat[ImmutableBytesWritable]])
//
//          // step 2: rdd mapping to table
//
//          // 在 HBase 中表的 schema 一般是这样的
...
//          // 我们需要将 *RDD[(uid:Int, name:String, age:Int)]* 转换成 *RDD[(ImmutableBytesWritable, Put)]*
//          // 我们定义了 convert 函数做这个转换工作
//          def convertFirstCity(triple: (PassInfoForEsHbase)) = {
//            var input = triple
...
//          }
//
//          var aaa = aa.filter { x => x.A017 > 0 }.map(convertFirstCity)
//
//          aaa.saveAsNewAPIHadoopDataset(jobFirstCity.getConfiguration())
          ...
          println(nowDate + " rdd的长度 : " + aaCount)
          // 初次入城入HBase-End
