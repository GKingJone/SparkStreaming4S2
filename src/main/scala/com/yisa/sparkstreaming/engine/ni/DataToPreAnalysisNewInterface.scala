 ...
 * plateTypeId
 */
object DataToPreAnalysisNewInterface {
  ...
  def main(args: Array[String]) {
    ...
    var preTopic = "recognitionMessage"
    
    var sparkConf = new SparkConf().setAppName("DataToPreAnalysisNewInterface")
    ...
    val topics = preTopic // Config10.KAFKA_TOPIC
      ...
      if (!rdd.isEmpty()) {
        ...
        var aa = rdd.coalesce(50, true).map(line_data => {
          ...
          if (true) {
            ...
            var nowDate = System.currentTimeMillis()/1000
            if (plateNumber != null && !"".equals(plateNumber) && !plateNumber.contains("无")
              ...
              // 查询Hbase
              val hbaseConn = HBaseConnectManager.getConnet(zk_old)
