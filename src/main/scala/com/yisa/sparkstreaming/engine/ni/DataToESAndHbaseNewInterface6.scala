 ...
 */
object DataToESAndHbaseNewInterface6 {

  def main(args: Array[String]) {

    // hbase表名
    var hbaseTableNameForEs: String = "vehicleinfosForES1"
    ...
    var sparkConf = new SparkConf().setAppName("DataToESAndHbaseNewInterface6")
    ...
    messages.foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        ...
        if (!aa.isEmpty()) {
          ...
          if (true) {
            aa.foreachPartition { x =>

              var hbaseConn = HBaseConnectManager.getConnet(zk_old)
              var table = hbaseConn.getTable(TableName.valueOf(hbaseTableNameForEs));
