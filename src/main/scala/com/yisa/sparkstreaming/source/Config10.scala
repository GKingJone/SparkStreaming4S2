    ...
    if (zkUtil == null) {
      ...
      synchronized {
        if (zkUtil == null) {
          ...
          // 预警接口
//          XD_URL = zkUtil.getConfig(ZK_SERVER_UT, ZK_TYPE, "xd_url").toString()
//          
//          // 套牌接口
//          TP_URL = zkUtil.getConfig(ZK_SERVER_UT, ZK_TYPE, "tp_url").toString()
  ...
  def showIndexString(): String = {
    ...
    sb.append("Spark UI 端口 ： ").append(SPARK_UI_PORT).append("\n")
    sb.toString()
